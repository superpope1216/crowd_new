...
<%
	String tabValue=StringUtil.toStr(request.getAttribute("tabValue"));
	
		
		
%>
<%!public String setTabActive(String _tabValue,String _tabSelectValue) {
		if (StringUtil.isEmpty(_tabSelectValue) && "0".equals(_tabValue)) {
			return " active ";
		}
		if (_tabValue.equals(_tabSelectValue)) {
			return " active ";
		}
		return "";
	}%>
...
<body>
						...
						<!-- <input type="button" value="发送" id="btnSend"> -->
							<div class="mynav">
								<ul class="nav nav-tabs nav-tabs-line m-b-0 shop-order-state" style="width:3999px;transform: translate3d(0px, 0px, 0px);">
									...
									<li class='nav-item <%=setTabActive("0",tabValue) %>'><a class="nav-link"
										data-toggle="tab" href data-state="0">全部(${cg+dsh+zbz+jxz+yjs})</a></li>
									<li class='nav-item <%=setTabActive("1",tabValue) %>'><a class="nav-link"
										data-toggle="tab" href data-state="1">草稿(${cg})</a></li>
									<li class='nav-item <%=setTabActive("2",tabValue) %>'><a class="nav-link" data-toggle="tab"
										href="#state1" data-state="2">待审核(${dsh})</a></li>
									<li class='nav-item <%=setTabActive("6",tabValue) %>'><a class="nav-link" data-toggle="tab"
										href="#state1" data-state="6">审核不通过(${shbtg})</a></li>
									<li class='nav-item <%=setTabActive("3",tabValue) %>'><a class="nav-link" data-toggle="tab"
										href="#state1" data-state="3">招标中(${zbz})</a></li>
									<li class='nav-item <%=setTabActive("4",tabValue) %>'><a class="nav-link" data-toggle="tab"
										href="#state0" data-state="4">进行中(${jxz})</a></li>
									<li class='nav-item <%=setTabActive("5",tabValue) %>'><a class="nav-link" data-toggle="tab"
	...
	<script>
		var page_type = "xqfOrder";
		var parentId = "${parentId}";
		var date="${date}";
		var rwxb="${rwxb}";//查询有投标人数的需求标注
		var xmzt="${rwState}";
	</script>
