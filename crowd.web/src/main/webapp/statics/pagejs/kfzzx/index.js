...
define(function(require, exports, module) {
	...
	inithtml();
	 ...
	 getDevArea();
	 function getDevArea(){
		 doGet(basePath+"/kfzzx/devArea","",function(data){
			if(data && data.datas){
				var html="";
				for(var i=0;i<data.datas.length;i++){
					var _data=data.datas[i];
	 ...
	 function getTrainData(){
		 doGet(basePath+"/kfzzx/trainData","",function(data){
			if(data && data.datas){
				var xData=new Array();
				var yData=new Array();
				for(var i=0;i<data.datas.length;i++){
					var _data=data.datas[i];
					xData.push(_data.khyf);
					yData.push(toStr(_data.tgrs,0));
				}
				var option = {
					    grid: {
					        left: '3%',
			      ...
			      var myChart = echarts.init(document.getElementById('divTrainData'));
				myChart.setOption(option, true);
			} 
		 });
	 }
