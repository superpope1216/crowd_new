...
$(document).ready(function() {
 var isMobile = $('html').hasClass('isMobile') ? true: false;
    ...
    nav_width_fun();
    getMessageCount();
    var messgeTime=window.setTimeout(function() {
    	getMessageCount();
    },
    1000*2*60);
    function getMessageCount(){
    	doGet(basePath+"/demandMessage/queryNotReadCount","",function(data){
			if(data && data.datas){
				var _data=data.datas;
				if(_data>10){
					_data="10+";
				}
				$("#spanMessageCount").html(_data);
			}
    ...
    $("#btnAllSearch").click(function(){
    	window.location.href=basePath+"/center/index?flag=&condition="+$("#txtAllSearch").val();
    });
    $('#txtAllSearch').keydown(function(e){
    	if(e.keyCode==13){
    		if(e.preventDefault){
    			e.preventDefault();
    			}else{
