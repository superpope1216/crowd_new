...
public class AuthIsKfzInteceptor  extends HandlerInterceptorAdapter{
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        AuthIsKfzAnnotation auth=methodHandler.getMethodAnnotation(AuthIsKfzAnnotation.class);
        if(auth==null){
        	return true;
        }
        ...
        if(kfzxxObject!=null){
    		KfzxxInfoExtend kfzxxInfoExtend=(KfzxxInfoExtend)kfzxxObject;
    		if(kfzxxInfoExtend.getShzt().equals(YhxxShztEnum.CG.getCode())){
    			...
    			errCode=HttpCodeEnum.KFZ_SFRZ_PASSING.getCode();
    		}else if(kfzxxInfoExtend.getShzt().equals(YhxxShztEnum.WTG.getCode())){
    			errCode=HttpCodeEnum.KFZ_SFRZ_NOT_PASS.getCode();
