/**
 * Project Name:crowd.entity
 * File Name:KfzxyjfInfoExtend.java
 * Package Name:com.wisedu.crowd.entity.yhgl.extend
 * Date:2018年1月25日上午9:27:12
 * Copyright (c) 2018, 01113232@wisedu.com All Rights Reserved.
 *
*/

package com.wisedu.crowd.entity.yhgl.extend;

import java.io.Serializable;

import com.wisedu.crowd.entity.yhgl.KfzxyjfInfo;

/**
 * ClassName:KfzxyjfInfoExtend <br/>
 * Function: TODO ADD FUNCTION. <br/>
 * Reason:	 TODO ADD REASON. <br/>
 * Date:     2018年1月25日 上午9:27:12 <br/>
 * @author   dell
 * @version  
 * @since    JDK 1.6
 * @see 	 
 */
public class KfzxyjfInfoExtend extends KfzxyjfInfo implements Serializable{

	/**
	 * serialVersionUID:TODO(用一句话描述这个变量表示什么).
	 * @since JDK 1.6
	 */
	private static final long serialVersionUID = 1L;

}

