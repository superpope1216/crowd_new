...
public class YhjbxxInfoServiceImpl implements YhjbxxInfoService {
	...
	public DataResult<YhjbxxInfoExtend> login(String userName, String password, CustomOperateLog log) throws ServiceException {
	 ...
	 /**
	    ...
	    */
	@Override
	  public DataResult<YhjbxxInfoExtend> loginAdmin(String userName,String password,CustomOperateLog log) throws ServiceException{
		if (StringUtil.isEmpty(userName)) {
			throw new ServiceException("用户名称不能为空！");
		}
		if (StringUtil.isEmpty(password)) {
		...
		if (CommonUtil.isEmptyList(datas)) {
			...
			if(!YesNoEnum.YES.getCode().equals(yhjbxxInfoExtend.getSfyyglry())){
				throw new ServiceException("您不是平台管理员，请重新确认！");
			}
			return DataResult.success(yhjbxxInfoExtend);
		}
	...
	@Override
	public DataResult<Integer> selectCountByCondition(YhjbxxInfoExtend condition, CustomOperateLog log)
			throws ServiceException {
		return DataResult.success(yhjbxxInfoExtendMapper.selectCountByCondition(condition));
	}
