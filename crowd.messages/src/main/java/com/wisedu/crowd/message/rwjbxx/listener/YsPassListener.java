 ...
 *
 */
public class YsPassListener extends BaseListener implements SessionAwareMessageListener<Message>{

	@Autowired
	private RwjbxxInfoService rwjbxxInfoService;
	private final static Logger LOG = LoggerFactory.getLogger(YsPassListener.class);
	...
	private void sendMsg(String rwid) throws Exception {
		try {
			...
			this.sendToKfz(rwjbxxInfo, rwjbxxInfo.getKfzid(), "验收通过通知", SmsMessageTempleateEnum.YS_PASS.getName(), XxlxEnum.YSJG.getCode());
		} catch (Exception e) {
