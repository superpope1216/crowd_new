 ...
 *
 */
public class ShPassListener extends BaseListener implements SessionAwareMessageListener<Message>{

	@Autowired
	private RwjbxxInfoService rwjbxxInfoService;
	private final static Logger LOG = LoggerFactory.getLogger(ShPassListener.class);
	...
	private void sendMsg(String rwid) throws Exception {
		try {
			...
			this.sendToKfz(rwjbxxInfo, rwjbxxInfo.getKfzid(), "售后通过通知", SmsMessageTempleateEnum.SH_PASS.getName(), XxlxEnum.SHJSJG.getCode());
		} catch (Exception e) {
