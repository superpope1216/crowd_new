 ...
 *
 */
public class AutoShPassListener extends BaseListener implements SessionAwareMessageListener<Message>{

	@Autowired
	private RwjbxxInfoService rwjbxxInfoService;
	private final static Logger LOG = LoggerFactory.getLogger(AutoShPassListener.class);
	...
	private void sendMsg(String rwid) throws Exception {
		try{
			...
			this.sendToKfz(rwjbxxInfo, rwjbxxInfo.getKfzid(), "自动验收通过通知", SmsMessageTempleateEnum.AUTO_YS_PASS.getName(), XxlxEnum.YSJG.getCode());
			this.sendToXqf(rwjbxxInfo, rwjbxxInfo.getXqfid(), "自动验收通过通知", SmsMessageTempleateEnum.AUTO_YS_PASS.getName(), XxlxEnum.YSJG.getCode());
		} catch (Exception e) {
