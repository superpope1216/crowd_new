 ...
 *
 */
public class ShNotPassListener extends BaseListener implements SessionAwareMessageListener<Message>{

	@Autowired
	private RwjbxxInfoService rwjbxxInfoService;
	private final static Logger LOG = LoggerFactory.getLogger(ShNotPassListener.class);
	...
	private void sendMsg(String rwid) throws Exception {
		try {
			...
			this.sendToKfz(rwjbxxInfo, rwjbxxInfo.getKfzid(), "售后不通过通知", SmsMessageTempleateEnum.SH_NOTPASS.getName(), XxlxEnum.SHJSJG.getCode());
		} catch (Exception e) {
