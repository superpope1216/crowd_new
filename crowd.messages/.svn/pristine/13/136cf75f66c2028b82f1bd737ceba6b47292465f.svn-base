 ...
 *
 */
public class AutoYsNotPassListener extends BaseListener implements SessionAwareMessageListener<Message>{

	@Autowired
	private RwjbxxInfoService rwjbxxInfoService;
	private final static Logger LOG = LoggerFactory.getLogger(AutoYsNotPassListener.class);
	...
	private void sendMsg(String rwid) throws Exception {
		try {
			...
			this.sendToKfz(rwjbxxInfo, rwjbxxInfo.getKfzid(), "自动验收不通过通知", SmsMessageTempleateEnum.AUTO_YS_NOT_PASS.getName(), XxlxEnum.YSJG.getCode());
		} catch (Exception e) {
